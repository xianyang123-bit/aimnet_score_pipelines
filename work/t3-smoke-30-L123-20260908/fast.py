...
def submit(mode, lane=None, dependency=None):
    args = ["sbatch", "--parsable", "--job-name=t3-30-" + mode,
...
def dock_task_fast(task):
    ...
    cached = {}
    cached_path = td / "cached_poses.sdf"
    if cached_path.exists():
        for mol in Chem.SDMolSupplier(str(cached_path), removeHs=False):
            if mol is not None:
                mid = int(mol.GetProp("mol_id")) if mol.HasProp("mol_id") else int(mol.GetProp("_Name").rsplit(":",1)[-1])
                cached[mid] = mol
    missing = frame[~frame.mol_id.isin(cached)]
    ...
    if poses.exists() and poses.stat().st_size:
        for mol in Chem.SDMolSupplier(str(poses), removeHs=False):
            if mol is not None:
                mid = int(mol.GetProp("mol_id")) if mol.HasProp("mol_id") else int(mol.GetProp("_Name").rsplit(":",1)[-1])
                cached[mid] = mol
    poses = ad / "poses.sdf"
    available = [int(mid) for mid in frame.mol_id if mid in cached]
    if available:
        writer = Chem.SDWriter(str(poses))
        for mid in available:
            writer.write(cached[mid])
        writer.close()
    publish(task, {"poses": str(poses), "failed": failed, "reused_cached_poses": len(frame)-len(missing)})
...
def prepare_fast():
    ...
    groups = [[r for r in records if r["layer"]==layer and r["status"]=="ready"] for layer in ("L1","L2","L3")]
    tasks=[]
    ...
    for task in tasks:
        if (chunk_dir(task)/"complete.json").exists():
            maybe_summarize(task)
    atomic_json(STATE/"benchmark.json",{"status":"Previously validated serial scoring protocol",
        "streams_per_gpu":1,"production_omp_threads":4,"new_benchmark_run":False})
    cpu=[{"lane":i,"job_id":submit("cpu",i)} for i in range(CPU_WORKERS)]
    gpu=[{"lane":i,"job_id":submit("gpu",i)} for i in range(GPU_WORKERS)]
    atomic_json(HOME/"launch.json",{"workers":gpu,"cpu_workers":cpu,
        "implementation":"fast.py","state":str(STATE),"scope":"5 actives + 25 decoys per target"})
    log("SMALL_RUN_LAUNCHED",len(tasks),"chunks")
